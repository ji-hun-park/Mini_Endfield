...
namespace Physics
{
    // Opaque handle to a physics SDK object (body, shape, joint, world, etc.).
    // The backend interprets this pointer; the engine treats it as an opaque token.
    using SDKObjectHandle = void*;

    // 64-bit opaque identifier mirroring UnityEngine's internal EntityId type.
    // Used to map physics objects back to their engine-side GameObjects/Components.
    ...
    struct EngineObjectId
    {
        ...
        {
            static_assert(sizeof(T) == sizeof(EngineObjectId), "Target type needs to have the same size (8 byte) as the EngineObjectId type");
    ...
    // Bitmask of optional features that a physics backend integration can support.
    // The engine queries these flags to determine which components and APIs are available.
    ...
    // Per-joint-type limit ranges reported by the backend as part of its integration info.
    struct IntegrationLimits
    {
        JointLimitRange joints[(size_t)JointType::Count];
    };

    // Metadata describing the physics backend integration, including its unique ID, version numbers,
    // underlying SDK version, supported feature set, human-readable name/description, and joint limits.
    // Returned by the GetIntegrationInfo command.
    struct IntegrationInfo
    {
        ...
        IntegrationLimits limit;
    };

    // Controls the verbosity of error and warning messages reported by the physics backend.
