...
namespace Physics
{
    // Compact 16-byte collision filter data attached to every shape. Encodes layer membership,
    // collision mask, pair event flags, CCD mode, geometry type, layer override priority,
    // trigger state, vehicle ID, and the ignore-pair ID for Physics.IgnoreCollision.
    // This struct is passed to the broadphase filter shader to determine which shape pairs collide.
    struct FilterData
    {
        FilterData()
            : belongsTo(0)
