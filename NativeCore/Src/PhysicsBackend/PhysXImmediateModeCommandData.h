...
namespace PhysicsCommands
{
    namespace PhysXExt
    {
        // Commands for PhysX immediate mode physics, which bypasses the scene and performs
        // low-level collision detection directly on geometry pairs without a simulation step.
        namespace ImmediateModeData
        {
            // Retrieves the PhysX-native geometry holder (PxGeometryHolder) for a shape, used as input for immediate mode collision queries.
            ...
            // Generates contacts between paired geometry arrays using PhysX immediate mode.
            // Takes two arrays of geometry/transform pairs and produces contact points without requiring a scene simulation step.
            // 'sizes' receives the per-pair contact count; 'writtenContacts' reports the total contacts written.
