...
namespace PhysicsCommands
{
    namespace PhysXExt
    {
        // PhysX-specific extension commands that go beyond the generic physics command protocol.
        // These are dispatched using the kInternalReservedMask0 category and cover PhysX scene management,
        // character controllers, vehicle simulation, immediate mode collision, and contact modification queries.
        enum class PhysXExtension : uint32_t
        {
            Invalid,

            // Scene — rebuilds the broadphase MBP region grid
            RebuildSceneMBPRegions,

            // Character controller — capsule-based kinematic controller with slope/step limits and collision feedback
            ...
            // Vehicle — PhysX vehicle SDK wrapper for wheeled vehicle simulation (up to kMaxVehicleWheelsCount wheels)
            CreateVehicle,
            DestroyVehicle,
            ...
            // Immediate mode and contact modification — low-level geometry queries and per-contact-pair data access
