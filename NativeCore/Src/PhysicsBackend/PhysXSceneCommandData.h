...
namespace PhysicsCommands
{
    namespace PhysXExt
    {
        namespace SceneData
        {
            // Rebuilds the Multi-Box Pruning (MBP) broadphase regions for the scene.
            // Used when the broadphase type is MBP to redefine the world bounds and subdivision grid.
            struct RebuildSceneMBPRegions : Command
            {
                static constexpr auto command = PhysXExtension::RebuildSceneMBPRegions;

                Physics::Vec3 min = { -256.0f, -256.0f, -256.0f };
                Physics::Vec3 max = { 256.0f, 256.0f, 256.0f };
