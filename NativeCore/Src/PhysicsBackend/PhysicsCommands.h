...
namespace PhysicsCommands
{
    ...
    constexpr uint32_t kPhysXExtensionMask = 0x8 << kMaskOffsetSize;
    constexpr uint32_t kFullMask = 0xF << kMaskOffsetSize;
    constexpr uint32_t kReverseFullMask = ~kFullMask;

    // SDK-level commands for global physics backend lifecycle, visual debugger connectivity, and material management.
    ...
    // World-level commands for creating/destroying physics scenes, configuring simulation parameters,
    // stepping the simulation, retrieving results, and managing the lifecycle of bodies, articulations, and joints.
    ...
    // Per-body commands for manipulating rigid body properties: flags, mass, damping, pose, shapes,
    // forces, velocities, inertia, collision detection mode, kinematic targets, and sleep state.
    ...
    // Per-shape commands for creating/destroying collision shapes, configuring geometry, filter data,
    // materials, poses, bounds, collision ignore pairs, mesh cooking, and height field management.
    ...
    // Scene query commands for raycasts, shape casts (sweeps), overlap tests, and body casts.
    // Each query type has single-hit and collector-callback variants.
    ...
    // Per-joint commands for configuring constraint properties: flags, anchor poses, break thresholds,
    // 6-DOF limits/motors, distance limits, hinge limits/motors, and connected actor management.
    ...
    // Articulation commands spanning three sub-domains: per-link body properties (mirroring Body commands),
    // per-link joint configuration (type, limits, motors, reduced-space coordinates), and whole-articulation
    // operations (link hierarchy, sleep state, solver iterations, bulk joint data access via float buffers).
    ...
    // Empty base struct for all command data types. Each concrete command struct inherits from Command
    // and carries the input/output fields for a single physics operation.
    struct Command {};

    // Provides the world and body handles that scope a command invocation.
    // 'world' identifies the physics scene; 'body' identifies the target actor (or null for world-level commands).
    struct Context
    {
        constexpr Context() = default;
        constexpr Context(Physics::SDKObjectHandle world_, Physics::SDKObjectHandle body_)
            : world(world_), body(body_)
        {
        }
    ...
    // Top-level entry point function signature for the physics backend. Dispatches an encoded command
    // (category mask + command index) to the appropriate handler, passing the SDK object, context, and command data.
    using ProcessCommandFunc = void (*)(Physics::SDKObjectHandle sdkObject, const Context& ctx, const uint32_t cmdFunc, Command& c);

    // Per-command handler function signature. Invoked by the backend's ProcessCommandFunc after
    // stripping the category mask, with the SDK object, context, and command data.
