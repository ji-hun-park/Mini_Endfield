...
namespace Physics
{
    // Classifies a physics actor by its simulation role.
    enum class ActorType : uint16_t
    {
        Invalid,
        Static,   // Immovable collider (e.g. terrain, walls)
        Dynamic,  // Rigidbody that participates in dynamics
        Link      // An articulation link within an articulation hierarchy
    };

    // Lightweight descriptor that identifies a physics actor by its SDK handle, engine-side user data,
    // and type. Mainly used in joint commands to reference the actors on each side of a constraint.
