...
namespace Physics
{
    // A 3-component float vector (x, y, z) used throughout the physics command layer for positions,
    // directions, velocities, forces, and scales. Provides zero-copy Load/Store/AsRef helpers for
    // type-punning with other same-sized float3 types (e.g. Unity's Vector3f).
    struct Vec3
    {
    ...
    // A 4-component float vector (x, y, z, w) used for quaternion rotations and four-element data.
    // Provides the same zero-copy Load/Store/AsRef helpers as Vec3, with an XYZW data layout convention.
    struct Vec4
    {
        // Default ctor does not initialize data
        Vec4() = default;
